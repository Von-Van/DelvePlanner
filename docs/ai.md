...
Delve Planner has one AI feature: a natural-language planner that turns a request such as
...
Everything except the planner. Today and Week, plans, milestones, workstreams, people, the Inbox, Plan the Week and Plan Today, carry-forward, calendars, time blocks, capacity, reminders, export and import, and **What Delve Planner knows** all run without a model. Its observations (**What it noticed**) are deterministic statistics computed in Rust from local records ([`db/profile.rs`](../src-tauri/src/db/profile.rs)), not model output.
...
| Creating workstreams and putting tasks in them                                                     | Implemented (v0.4.0)                                          |
| Turning inbox items into tasks, events, or plans                                                   | Implemented (v0.4.0)                                          |
| Editing a suggestion's details before applying it                                                  | Implemented (v0.4.0)                                          |
| Choosing a day around spare time, days off, and the daily limit                                    | Implemented (v0.4.0), only when asked to choose               |
...
| Preferred hours, focus and break lengths, or energy preference in suggestions                      | Planned; the planner doesn't see them                         |
| Cloud models or a remote fallback                                                                  | Not planned                                                   |

## Where the model runs

All inference is local. Delve Planner bundles the [Ollama](https://github.com/ollama/ollama) server (version 0.32.0, pinned and checksum-verified at packaging) and manages it from [`runtime.rs`](../src-tauri/src/runtime.rs):

- **Private endpoint.** The server is a child process listening on `127.0.0.1` at a port Delve Planner reserves at startup. A system Ollama on its usual port 11434 is ignored. Delve Planner refuses to use a server that reports any version other than the bundled one.
...
- **Quitting means quitting.** Ollama runs a model in a separate runner process, so Delve Planner ends the whole tree on exit: the server's process group on macOS, `taskkill /T` on Windows. A Delve Planner that crashed records its server's process ID, and the next launch ends that server (after checking it is still Delve Planner's runtime binary) before starting another.
- **Models.** Delve Planner lists the models it downloaded (under its app data, `ai-models/`) alongside the machine's own Ollama models (`OLLAMA_MODELS`, or `~/.ollama/models`), which it only reads. The one model it downloads is `qwen3:8b` (about 5.2 GB), after the user approves; the digest is checked before AI is enabled. Removing model data deletes only Delve Planner's own folder.
...
sequenceDiagram
  ...
  UI->>Agent: propose_schedule_changes(command, day, timeZone, activePlanId, weekStartsOn)
  Agent->>DB: Rank candidate events, plans, milestones, tasks, and workstreams
  DB-->>Agent: Relevant records only (and spare time, when asked to choose a day)
  ...
  User->>UI: Accept some or all suggestions, editing any details
  UI->>Agent: apply_schedule_changes(proposalId, accepted, edits)
  Agent->>DB: Check edits keep their targets, recheck expiry and revisions, apply one transaction
...
The renderer cannot submit invented changes. `PlannerAgent` keeps one pending proposal per in-memory session for ten minutes. Applying accepts only its opaque `proposalId`, the handles of the suggestions the user accepted, and any suggestions the user edited; Rust retrieves the validated operations, checks that each edit changes only values — never the record it targets, the plan, milestone, or workstream it links to, the inbox item it comes from, or the revision it was proposed against — rechecks IDs and revisions, and consumes the proposal after one attempt.

A proposal holds up to twelve operations from a closed set:

| Operation             | Typed fields                                                                                                                             |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `create_event`        | title, notes, UTC start, IANA time zone, duration, optional reminder offset, optional plan, optional inbox item                          |
...
| `create_plan`         | title, description, status, optional start and target dates, optional inbox item                                                         |
...
| `create_task`         | title, description, optional plan, milestone, and workstream, optional scheduled day and due date, status, priority, optional inbox item |
...
| `create_workstream`   | plan, name                                                                                                                               |
| `set_task_workstream` | task ID + revision, workstream or none                                                                                                   |

The Rust definition is `MutationOperation` in [`model.rs`](../src-tauri/src/model.rs).

- **References.** A plan, milestone, or workstream reference is either an existing ID or the name of one created earlier in the same proposal, so “start a Bake sale plan and add a Buy flour task to it” is one reviewable change. Names match ignoring case and spacing, the same way in review and when applying. Plans, then milestones, then workstreams are created first when a proposal applies. A workstream must belong to its task's plan; a task given only a workstream joins that workstream's plan.
- **Inbox items.** A new task, event, or plan can name the inbox item it comes from. Applying removes the item in the same transaction, and an item edited or removed since the proposal was made fails the whole proposal. The planner only turns inbox items into anything when the request mentions the inbox.
- **Edits before applying.** In review, a suggestion's details — a title or name, a day or due date, an event's start and length, an estimate, a target date — can be changed before applying. Rust accepts an edit only when everything else about the suggestion is exactly as proposed, carries a renamed new plan, milestone, or workstream to the suggestions that refer to it, and validates the edited proposal like any other.
...
- **Reasons and suggestions.** `create_task`, `update_task`, and `schedule_task` may carry a one-line `reason`. The planner picks a day, due date, or week itself only when the request asks it to (“when should I…”, “find time for…”, “plan my week”), and those choices are labelled **suggested** in the review. Otherwise an invented date is dropped or turned into a question. A day it picks that has no working time — outside working hours or a day off in the planning profile — moves to the next day that has working time, with a reason saying so; if none of the next 14 days has any, it asks instead.
- **Constrained output.** The model answers through Ollama structured outputs: Delve Planner sends a JSON Schema that Ollama compiles into a decoding grammar, so a reply cannot contain unknown operations or fields, and ID fields accept only the IDs of records in that request's context.
...
- **Clarifications instead of guesses.** Ambiguous titles (including duplicates), missing targets and dates, deleted records, a workstream that doesn't exist, bare 12-hour times such as `at 2`, daylight saving gaps and overlaps, repeats, task reminders, deleting or archiving plans, assigning people or locations, requests that address the model's instructions or name its internal operations, and contradictory compound requests all produce a question.
...
What the planner deliberately can't do: delete or archive plans, assign owners, rename or delete workstreams, change locations, set a task to repeat, edit checklists, or add task reminders. Those stay manual.

## What the model sees

Context is deliberately small, and it is built in Rust ([`prompt.rs`](../src-tauri/src/agent/prompt.rs), `request_context`):

- The request text, today's and tomorrow's dates, this and next week (starting on the day the user's weeks start on), the time zone name, the dates of the next seven weekdays, and a 21-day list of dates.
- Up to 40 Delve Planner **events**, ranked by whether the session referred to them, words shared with the request, and closeness to the selected day: ID, title, local start, duration, reminder offset, and (for planning requests) plan ID.
- For planning requests only — those that use words such as “plan”, “task”, or “milestone” or name one, are made while a plan is open, or follow a planning request:
  ...
  - up to 20 **milestones** and 30 **tasks** that match the request's words, belong to a plan the request names or the one being viewed, were referenced earlier in the session, or (for tasks) are scheduled or due on the selected day or the next: IDs, titles, plan, milestone, and workstream links, scheduled day, due date, status, and non-default priority;
  - up to 40 **workstreams** of those plans: ID, plan ID, and name;
  - when the request mentions the inbox, up to 20 **inbox items**: ID and captured text;
  - when the request asks the planner to choose a day, the **spare time** on each of the next 14 days: one number per day, the working time left once Delve Planner events, busy time on other calendars, and the estimates of work already scheduled are taken out, capped by the daily limit; days with no working time are left out;
...
It never sees event notes or locations; plan, milestone, task, or workstream descriptions; inbox notes; individual estimates; people; time blocks; other apps' calendar events; the planning profile's preferred hours, focus and break lengths, or energy preference; or observations. Working hours, days off, the daily limit, busy calendar time, and estimates reach it only as the per-day spare-time number above, and only when it's asked to choose a day.
...
- **Logs:** Delve Planner never logs requests, replies, or proposal contents. In debug builds only, setting `DELVE_PLANNER_DEBUG_PLANNER=1` prints each raw model reply to stderr for investigating a failure.
- **Model files:** downloaded models stay in Delve Planner's app data until **Remove model**.
...
- [`eval/planning-cases.json`](../eval/planning-cases.json) — 58 planning cases on a shared fixture: creating and renaming plans, adding tasks to plans and milestones, moving tasks between plans, milestones, workstreams, inbox conversion, compound requests, follow-ups on pending and applied proposals, estimates and week choice, suggested dates and days off, ambiguity, deleted and stale targets, unsupported requests, and prompt injection in requests and in stored titles.
...
**Current evidence.** The committed results were generated on 2026-09-16 against `qwen3:8b` (digest `500a1f06…`) and Ollama 0.32.0: all three runs passed every gate on the 111 cases that existed then. Seven planning cases were added in v0.3.2 and eight in v0.4.0, so the current 126-case set has not yet been recorded in a full three-run gate.
