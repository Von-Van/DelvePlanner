...
Delve Planner is for one person with several things going on — a launch, a move, a trip, a research project, a charity stream. Plans, milestones, and tasks sit beside a Today and Week agenda that also shows your Google, Outlook, and other calendars, read-only. An optional AI planner, running entirely on your machine, turns a messy request into changes you review before anything is saved.

> “Move gym to 6pm tomorrow, mark Book venue done, and add a Rent cameras task to Charity Week due October 9.”

Delve Planner (formerly DayPlan) runs on macOS and Windows. There is no Delve Planner account, no server, and no cloud AI.

[Releases](https://github.com/Von-Van/DelvePlanner/releases) · [Architecture](docs/architecture.md) · [Privacy](docs/privacy.md) · [AI planner](docs/ai.md) · [Roadmap](ROADMAP.md) · [Changelog](CHANGELOG.md)
...
Delve Planner explores the space in between:
...
- **Plans are optional.** Anything without a plan is still first-class, so Delve Planner also works as a plain day planner.
- **Light by design.** A closed set of statuses; no custom fields, sprints, story points, Gantt editing, or automation. People are labels, not accounts. Tasks can repeat and wait on one another, but lightly: one open occurrence at a time, and a wait is a hint beside the task, never a lock.
- **Nothing moves on its own.** Delve Planner shows planned work against available time and surfaces unfinished work, but you decide what moves.
- **AI assists; it never decides.** The planner proposes changes, and you accept or reject each one.

## Features

The lists below describe the current source (v0.4.0). The [changelog](CHANGELOG.md) says which version added what, and [Releases](https://github.com/Von-Van/DelvePlanner/releases) lists the published builds.

### Available

| Area                     | What you get                                                                                                                                                                                                  |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Today and Week           | Events, tasks scheduled or due, milestones, time blocks, and other calendars' events side by side, with a remembered plan filter and your own week start                                                      |
| Plans                    | Status, dates, color, and links; milestones and workstreams; templates and duplication; an overview with attention signals, a timeline, a schedule, and a run of show for event days                          |
| Tasks                    | Repeating tasks, checklists, and waits on other tasks, alongside estimates, priorities, and owners                                                                                                            |
| Capture                  | An Inbox, quick capture inside Delve Planner (⌘I / Ctrl+I) and — with a shortcut you record — from any app; items become tasks, plans, or events when you're ready                                            |
...
| Reminders                | One reminder per event, delivered by the OS while Delve Planner runs in the tray                                                                                                                              |
| AI planner               | Natural-language requests become proposals for events, plans, milestones, tasks, and workstreams — including from your inbox — that you edit, accept, or reject one suggestion at a time and apply atomically |
| What Delve Planner knows | A planning profile you set, and six observations computed from your own records, each showing its evidence and with an off switch                                                                             |
...
- **Google and Outlook sign-in.** Connections work, but Delve Planner's OAuth apps aren't verified yet: Google shows an "unverified app" warning, and work or school Microsoft accounts often need an administrator's approval. Subscribing by link works for any calendar.
...
- The AI planner using the rest of your planning profile. It already works around days off, your daily limit, and busy time when you ask it to pick a day; preferred hours, focus and break lengths, and energy preference are recorded but not used yet.
...
Delve Planner is a [Tauri 2](https://v2.tauri.app/) app. The React interface is deliberately the least-trusted layer: it calls a narrow set of typed commands and never touches SQL, secrets, files, or the network. Rust owns validation, revision-checked transactions, migrations, calendar sync, the local model's lifecycle, and native integrations such as notifications and the tray.
...
**Delve Planner goes online only when you:**

- add a calendar — Delve Planner reads it from the provider or the link's host over HTTPS;
- approve the model download — the bundled Ollama fetches `qwen3:8b` from Ollama's registry;
- select **Check for updates** — Delve Planner asks GitHub for the latest release; or
- open a link, such as the model's license page, in your browser.

Delve Planner has no telemetry or analytics. The model runs locally and never sees notes, descriptions, locations, people, or other apps' calendar events; when you ask it to pick a day, it sees one spare-time number per day, which busy calendar time counts toward. [Privacy](docs/privacy.md) is the full, code-referenced inventory.

## Calendar integration

Calendars flow one way — into Delve Planner — and stay read-only.
...
Every request to a calendar API is a `GET`. Refresh tokens and private links go to the macOS Keychain or Windows Credential Manager; the interface never sees them. Events from other calendars count toward busy time but are never edited, exported, or sent to the AI planner; their busy time reaches it only as part of a day's spare-time number when you ask it to pick a day. Disconnecting revokes access where the provider allows and deletes everything cached. [Calendar integration](docs/calendar-integration.md) covers sync, caching, recurrence, and failure handling, and [calendar accounts](docs/calendar-accounts.md) covers the OAuth clients.

## The AI planner

The planner runs a bundled, version-pinned Ollama server on a private loopback port and uses `qwen3:8b` (a 5.2 GB download you approve) or a model already installed on the machine. The runtime starts only when it's needed — a planner request, a model download, or a model check — and stops after five idle minutes; quitting Delve Planner ends it.

Replies are constrained by a JSON Schema grammar to seventeen typed operations, validated again in Rust, and shown as a preview. Ambiguous requests get a question, not a guess. An evaluation harness scores the planner on 126 hand-labeled requests — including daylight saving edge cases and prompt injection — three runs at a time against the tested model. [The AI planner](docs/ai.md) explains the permission boundary, what the model sees, and what the latest recorded results cover.

## Running Delve Planner

### Install

Download the latest build from [Releases](https://github.com/Von-Van/DelvePlanner/releases): a universal DMG for macOS 13 or later, or a setup `.exe` for Windows 10 22H2 or 11 (x64). Builds are unsigned pre-releases for now, so macOS asks you to approve the app in **System Settings → Privacy & Security** and Windows SmartScreen asks for confirmation on first launch. For the AI planner, 16 GB of RAM and about 10 GB of free disk space are recommended.
...
Development builds share their data folder with an installed Delve Planner. To use the AI planner in development, fetch the pinned runtime first with `bash scripts/fetch-ollama-runtime.sh` (macOS). [Development](docs/development.md) covers the runtime, every command, the evaluation harness, and releasing.
...
## How Delve Planner is built

Delve Planner is developed with AI coding assistants, including Anthropic's Claude through Claude Code, which is credited as a co-author on the release commits it helped write. The project owner sets the product direction, requirements, architecture, privacy and integration decisions, and reviews and accepts every change; the roadmap's recorded decisions, the tests, the evaluation gates, and the release checklist are where that judgment is written down. [Contributing](CONTRIBUTING.md#ai-assisted-development) describes the workflow.

Delve Planner started in February 2026 as a Flask web app called DayPlan, became a SwiftUI iOS app with widgets in June — preserved on the [`ios-swiftui`](https://github.com/Von-Van/DelvePlanner/tree/ios-swiftui) branch, with its own separate data — was rebuilt as this desktop app in August, and took its current name in September.
